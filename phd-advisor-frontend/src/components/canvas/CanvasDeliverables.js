...
export const TEMPLATES = [
  {
    id: 'research-paper',
    name: 'Internship Search Plan',
    desc: 'Goals → Target roles → Channels → Weekly cadence → Materials → Tracking',
    icon: 'book',
    mode: 'paper',
    sections: [
      { id: 'abstract', name: 'Overview', target: 250, hint: 'One paragraph: academic standing, target term, role lane, and success metric.', checks: ['hasNumber', 'hasFinding'] },
  ...
  },
  {
    ...
    desc: 'Overview → What you applied to → Callbacks → What to change',
    icon: 'book',
    mode: 'paper',
    sections: [
      { id: 'overview', name: 'Overview', target: 200, hint: 'What week this covers and the goal.', checks: [] },
      { id: 'background', name: 'Applications sent', target: 1500, hint: 'Companies, roles, and channels used.', checks: ['hasNumber'] },
      { id: 'methods', name: 'Outreach & prep', target: 1500, hint: 'Alumni notes, career-fair follow-ups, interview practice.', checks: ['hasNumber'] },
      { id: 'results', name: 'Results', target: 2000, hint: 'Screens, interviews, and feedback received.', checks: ['hasNumber'] },
      { id: 'discussion', name: 'What to change', target: 1500, hint: 'What worked, what did not, next week adjustments.', checks: ['hasLimit'] },
    ],
  },
  {
    id: 'nsf-grfp',
    name: 'Career Intake',
    desc: 'Student background + goal plan for advising.',
    icon: 'award',
    mode: 'document',
    sections: [
      { id: 'personal', name: 'Student background', target: 1500, hint: 'Major, year, experience, and motivation. Write as a story.', checks: [] },
      { id: 'research', name: 'Goal plan', target: 1000, hint: 'Specific target (role/term) and how you will get there.', checks: ['hasNumber'] },
    ],
  },
  {
    id: 'conference-abstract',
    name: 'Weekly Check-in',
    desc: 'Single section, ~250 words. Lead with pipeline numbers.',
    icon: 'send',
    mode: 'document',
    sections: [
      { id: 'abs', name: 'Check-in', target: 250, hint: 'One paragraph: apps sent, interviews, blockers, next focus.', checks: ['hasFinding', 'hasNumber'] },
    ],
  },
  {
    id: 'defense-slides',
    name: 'Search Presentation',
    desc: 'Title → Goals → Targets → Cadence → Materials → Risks → Q&A',
    icon: 'kanban',
    mode: 'slides',
    sections: [
      { id: 'title', name: 'Title slide', target: 30, hint: 'Search name, your name, date.', checks: [] },
      { id: 'outline', name: 'Outline', target: 60, hint: '5–7 bullets covering the plan.', checks: [] },
      { id: 'background', name: 'Background', target: 200, hint: 'Standing, major, and why this lane.', checks: [] },
      { id: 'question', name: 'Goal', target: 80, hint: 'Single measurable goal for the term.', checks: [] },
      { id: 'methods', name: 'Approach', target: 200, hint: 'Channels, cadence, and outreach plan.', checks: ['hasNumber'] },
      { id: 'results', name: 'Expected results', target: 300, hint: 'Realistic pipeline targets by month.', checks: ['hasNumber'] },
      { id: 'discussion', name: 'Risks', target: 200, hint: 'Time conflicts, thin experience, location limits.', checks: ['hasLimit'] },
      { id: 'qa', name: 'Anticipated Q&A', target: 300, hint: 'Hardest 5 questions and your answers.', checks: [] },
    ],
  },
  {
    id: 'poster',
    name: 'Pipeline Poster',
    desc: '4-quadrant board: Goal · Approach · Results · Next.',
    icon: 'layout',
    mode: 'poster',
    sections: [
      { id: 'title', name: 'Title & Student', target: 30, hint: 'Search focus, your name, term.', checks: [] },
      { id: 'intro', name: 'Goal', target: 200, hint: 'Where you started and where you are headed.', checks: [] },
      { id: 'methods', name: 'Approach', target: 200, hint: 'Channels, cadence, materials.', checks: ['hasNumber'] },
      { id: 'results', name: 'Results', target: 250, hint: 'Headline pipeline numbers.', checks: ['hasNumber'] },
      { id: 'discussion', name: 'Next steps', target: 200, hint: 'What it means + what is next.', checks: ['hasLimit'] },
      { id: 'refs', name: 'Notes / Credits', target: 80, hint: 'Advisors, career services, links.', checks: [] },
    ],
  },
  {
    id: 'cv',
    name: 'Internship Résumé',
    desc: 'Standard sections: Education · Experience · Projects · Leadership · Skills.',
    icon: 'user',
    mode: 'document',
    sections: [
      { id: 'header', name: 'Header', target: 40, hint: 'Name, email, LinkedIn, city, phone.', checks: [] },
      { id: 'education', name: 'Education', target: 100, hint: 'School · Major · Expected grad · GPA (if strong).', checks: [] },
      { id: 'publications', name: 'Experience', target: 300, hint: 'Jobs/internships with action + metric bullets.', checks: ['hasNumber'] },
      { id: 'talks', name: 'Projects', target: 150, hint: 'Class or personal projects with tools and outcomes.', checks: [] },
      { id: 'awards', name: 'Leadership & activities', target: 100, hint: 'Clubs, mentoring, volunteer roles.', checks: [] },
      { id: 'service', name: 'Relevant coursework', target: 100, hint: 'Courses that match the target role.', checks: [] },
      { id: 'skills', name: 'Skills', target: 60, hint: 'Tools, languages, and platforms.', checks: [] },
    ],
  },
  {
    id: 'cover-letter',
    name: 'Cover Letter / Outreach',
    desc: 'For applications or alumni/recruiter outreach.',
    icon: 'send',
    mode: 'document',
    sections: [
      { id: 'header', name: 'Header', target: 40, hint: 'Date, recipient, greeting.', checks: [] },
      { id: 'opener', name: 'Opening paragraph', target: 100, hint: 'Role + why you are reaching out.', checks: [] },
      { id: 'body', name: 'Fit & proof', target: 250, hint: '2–3 proof points tied to the JD. Numbers > adjectives.', checks: ['hasNumber'] },
      { id: 'fit', name: 'Why this org', target: 150, hint: 'Specific company/team reason.', checks: [] },
  ...
  },
  {
    ...
    desc: 'Company research, likely questions, and STAR answers.',
    icon: 'shield',
    mode: 'document',
    sections: [
      { id: 'overview', name: 'Interview overview', target: 200, hint: 'Company, role, format (phone/virtual/onsite), date.', checks: [] },
      { id: 'background', name: 'Company research', target: 400, hint: 'Product, recent news, values, and why you care.', checks: [] },
      { id: 'aims', name: 'Role must-haves', target: 250, hint: 'Keywords and requirements from the JD.', checks: [] },
      { id: 'participants', name: 'Your proof points', target: 300, hint: 'Match each must-have to a story or project.', checks: ['hasNumber'] },
      { id: 'procedures', name: 'STAR answers', target: 500, hint: 'Write 3–5 full STAR answers you will practice aloud.', checks: ['hasNumber'] },
      { id: 'risks', name: 'Hard questions', target: 200, hint: 'Gaps, GPA, job hopping, “tell me about a failure.”', checks: ['hasLimit'] },
      { id: 'benefits', name: 'Questions to ask them', target: 100, hint: '3 smart questions that show research.', checks: [] },
      { id: 'consent', name: 'Logistics', target: 200, hint: 'Outfit, tech check, arrival plan, thank-you note draft.', checks: [] },
      { id: 'data', name: 'Post-interview notes', target: 200, hint: 'What went well, what to improve, follow-up date.', checks: [] },
    ],
  },
  {
    id: 'meeting-prep',
    name: 'Advisor / Recruiter Meeting Prep',
    desc: 'Agenda, updates, decisions needed, follow-ups.',
    icon: 'message',
    mode: 'document',
    sections: [
      { id: 'agenda', name: 'Agenda', target: 80, hint: '3–5 bullets ranked by priority.', checks: [] },
      { id: 'progress', name: 'Progress since last meeting', target: 200, hint: 'Apps, interviews, outreach — with numbers.', checks: ['hasNumber'] },
      { id: 'blockers', name: 'Blockers', target: 150, hint: 'What is getting in the way.', checks: ['hasLimit'] },
      { id: 'decisions', name: 'Decisions needed', target: 200, hint: 'Frame as A/B options with your preference.', checks: [] },
      { id: 'questions', name: 'Questions', target: 150, hint: 'Open questions for the advisor/recruiter.', checks: [] },
      { id: 'followup', name: 'Action items', target: 100, hint: 'What to do + by when.', checks: [] },
    ],
  },
];
