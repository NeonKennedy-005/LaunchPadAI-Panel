...
KNOWN_TOOLS = {"get_current_datetime", "calculate_protein_target"}
...
class TestGetToolDefinitions(unittest.TestCase):
    ...
    def test_filter_to_single_tool(self):
        defs = get_tool_definitions(enabled=["current_datetime"])
        self.assertEqual(len(defs), 1)
        self.assertEqual(defs[0]["function"]["name"], "get_current_datetime")

    def test_module_alias_enables_nutrition_tool(self):
        defs = get_tool_definitions(enabled=["nutrition_calculator"])
        self.assertEqual(len(defs), 1)
        self.assertEqual(defs[0]["function"]["name"], "calculate_protein_target")
