...
def _make_session(user_message_count=1, prior_advisor=False):
    ...
    if prior_advisor:
        messages = [
            ...
            {"role": "nutrition_strategist", "content": "Aim for 1.6-2.2 g/kg. What is your body weight?"},
        ]
...
@patch("app.core.improved_orchestrator.get_settings")
class TestFollowUpClarificationSkip(unittest.TestCase):

    def _run(self, coro):
        return asyncio.run(coro)

    def test_skips_clarification_for_weight_follow_up_after_protein_question(self, mock_settings):
        mock_settings.return_value = _make_mock_settings()
        llm = MagicMock()
        llm.generate = AsyncMock()
        orch = _make_orchestrator(persona_llm=llm)
        session = _make_session(prior_advisor=True)
        session.messages.append({"role": "user", "content": "im 230 lbs"})

        result = self._run(
            orch.needs_clarification_improved(session, "im 230 lbs")
        )

        self.assertFalse(result)
        llm.generate.assert_not_called()

    def test_rule_based_skips_weight_follow_up_with_prior_topic(self, mock_settings):
        mock_settings.return_value = _make_mock_settings()
        orch = _make_orchestrator()
        session = _make_session(prior_advisor=True)
        session.messages.append({"role": "user", "content": "230 lbs"})

        result = orch.needs_clarification(session, "230 lbs")

        self.assertFalse(result)

    def test_skips_when_advisors_already_replied(self, mock_settings):
        mock_settings.return_value = _make_mock_settings()
        llm = MagicMock()
        llm.generate = AsyncMock()
        orch = _make_orchestrator(persona_llm=llm)
        ...
        session.messages = [
            ...
            {"role": "nutrition_strategist", "content": "Tell me your weight"},
            {"role": "user", "content": "230 lbs"},
        ]

        result = self._run(
            orch.needs_clarification_improved(session, "230 lbs")
        )
