...
def resolve_jwt_secret(explicit: str = "") -> str:
    """Resolve a stable JWT signing secret across restarts.

    Precedence: config value → JWT_SECRET_KEY env → persisted file under
    DATA_DIR (generated on first run) → insecure dev fallback.
    """
    ...
    try:
        ...
        logger.info("Generated persistent JWT secret at %s", secret_path)
        return generated
    except OSError as exc:
        logger.warning(
            "Cannot persist JWT secret at %s (%s); using insecure default. "
            "Set JWT_SECRET_KEY or auth.jwt_secret for production.",
            secret_path,
            exc,
        )
...
class AuthConfig(BaseModel):
    ...
    def _validate_jwt_secret(self):
        self.jwt_secret = resolve_jwt_secret(self.jwt_secret)
        return self
