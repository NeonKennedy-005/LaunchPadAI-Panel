...
# Short replies that supply data (weight, height, age) in response to a prior question.
_FOLLOW_UP_DATA_PATTERNS = [
    r"^\s*i['']?m\s+\d",
    r"^\s*i am\s+\d",
    r"\d+\s*(?:lbs?|pounds?|kg|kilos?|kgs)\b",
    r"^\s*\d{2,3}\s*(?:lbs?|pounds?|kg|kilos?|kgs)?\s*$",
    r"^\s*\d+\s*(?:years?\s*old|yo)\b",
    r"^\s*(?:about|around|roughly)\s+\d",
]

# Prior-turn topics that make a short data reply actionable (e.g. protein + weight).
_FOLLOW_UP_TOPIC_KEYWORDS = [
    "protein", "macro", "calorie", "weight", "body weight", "bulk", "cut",
    "recomp", "hypertrophy", "muscle", "nutrition", "meal", "diet",
    "how much", "how many", "grams", "g/day", "schedule", "days per week",
...
class ImprovedChatOrchestrator:
    ...
    def _get_user_messages(self, session: ConversationContext) -> List[Dict[str, str]]:
        return [msg for msg in session.messages if msg.get("role") == "user"]

    def _has_continuing_conversation(self, session: ConversationContext) -> bool:
        """True when the user is past the first turn or advisors have already replied."""
        user_messages = self._get_user_messages(session)
        if len(user_messages) > 1:
            return True
        advisor_roles = set(self.personas.keys()) | {"assistant", "orchestrator"}
        return any(msg.get("role") in advisor_roles for msg in session.messages)

    def _prior_conversation_text(
        self, session: ConversationContext, exclude_latest_user: bool = False
    ) -> str:
        """Concatenate prior messages for clarification and routing context."""
    ...
    def _is_follow_up_data_reply(
        self, session: ConversationContext, user_input: str
    ) -> bool:
        """
        Detect short replies that answer a prior question (e.g. weight after
        a protein question) even when session history failed to hydrate.
        """
        ...
        looks_like_data = any(
            ...
            for pattern in _FOLLOW_UP_DATA_PATTERNS
        )
        if not looks_like_data:
            return False

        prior_text = self._prior_conversation_text(
            session, exclude_latest_user=True
        ).lower()
        if not prior_text:
            return False

        return any(keyword in prior_text for keyword in _FOLLOW_UP_TOPIC_KEYWORDS)
    ...
    def needs_clarification(self, session: ConversationContext, user_input: str) -> bool:
        ...
        if self._has_continuing_conversation(session):
            logger.info("Skipping clarification: continuing conversation")
            return False

        if self._is_follow_up_data_reply(session, user_input):
            logger.info(
                "NO CLARIFICATION: follow-up data reply with prior topic context"
            )
            return False
    ...
    async def needs_clarification_improved(self, session: ConversationContext, user_input: str) -> bool:
        ...
        if self._is_follow_up_data_reply(session, user_input):
            logger.info(
                "Skipping clarification: follow-up data reply with prior topic context"
            )
            return False
        ...
        conversation_context = self._prior_conversation_text(
            session, exclude_latest_user=True
        )
        ...
        system_prompt = (
            ...
            "Your task: decide whether the user's latest message contains enough "
            ...
            "Use the full conversation history when provided. Short follow-up "
            ...
            "- Is a standalone data point with NO prior conversation context\n\n"
            ...
            "- Is a follow-up answer that completes information from earlier turns\n\n"
